        ...
        <div class="container mx-auto px-6">
            <div class="flex flex-col lg:flex-row gap-16">
                ...
                <div class="lg:w-1/2 space-y-4">
                    <div class="aspect-[4/5] overflow-hidden rounded-2xl bg-gray-50 border border-gray-100 shadow-sm">
                        @php
                    ...
                    <div class="grid grid-cols-4 gap-4">
                        ...
                        @foreach($product->gallery as $img)
                            ...
                            @endphp
                            <div class="aspect-square rounded-lg overflow-hidden border border-gray-200 cursor-pointer thumbnail opacity-60 hover:opacity-100 transition" onclick="changeImage('{{ $thumbUrl }}', this)">
                    ...
                    <div>
                        <nav class="flex text-sm text-gray-400 mb-4" aria-label="Breadcrumb">
                            <ol class="flex items-center space-x-2">
                                <li><a href="/" class="hover:text-black">Anasayfa</a></li>
                                <li><span class="mx-2">/</span></li>
                                <li><a href="/urunler" class="hover:text-black">Mağaza</a></li>
                                @if($product->category)
                                <li><span class="mx-2">/</span></li>
                                <li class="text-gray-600">{{ $product->category }}</li>
                                @endif
                            </ol>
                        </nav>
                        <h1 class="text-4xl md:text-5xl font-serif font-bold mb-4">{{ $product->title }}</h1>
                        <p class="text-3xl font-serif text-gray-900">{{ number_format($product->price, 0, ',', '.') }} TL</p>
                    </div>
    ...
    <script>
        function changeImage(src, el) {
            ...
            document.querySelectorAll('.thumbnail').forEach(function(item) {
                ...
                item.classList.add('border-gray-200', 'opacity-60');
            });
